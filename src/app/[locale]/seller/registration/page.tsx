...
// تم تعديل الاستيراد ليتوافق مع الـ Export الجديد في ملف OrderList
...
const SellerRegistrationForm = () => {
  ...
  // ... (باقي كود الـ State والدوال كما هو بدون أي تغيير جذري)
  ...
  useEffect(() => {
    ...
    const getUserSettingsData = async () => {
      try {
        ...
        setDbUserSettings(settings || null);
      } catch (error) {
  ...
  useEffect(() => {
    if (dbSeller) {
      ...
      setFormData({
        ...
        sellerDescription: t('SCREEN.SELLER_REGISTRATION.SELLER_DETAILS_PLACEHOLDER'),
        sellerAddress: t('SCREEN.SELLER_REGISTRATION.SELLER_ADDRESS_LOCATION_PLACEHOLDER'),
        email: '',
        phone_number: dbUserSettings?.phone_number || '',
        image: '',
        fulfillment_method: FulfillmentType.CollectionByBuyer,
  ...
  useEffect(() => {
    const { sellerName, sellerType, sellerDescription, sellerAddress } = formData;
    setIsFormValid(!!(sellerName && sellerType && sellerDescription && sellerAddress));
  }, [formData]);
  ...
  const handleChange = (e: any) => {
    ...
    setIsSaveEnabled(Object.values(updatedFormData).some((v) => v !== '') && isFormValid);
  };
    ...
    <div className="w-full md:w-[500px] md:mx-auto p-4">
      ...
      <div className="spacing-7">
          <ToggleCollapse header={t('SCREEN.SELLER_REGISTRATION.SELLER_ADVANCED_SETTINGS_LABEL')} open={true}>
              ...
              <FileInput label={t('SHARED.PHOTO.MISC_LABELS.SELLER_IMAGE_LABEL')} imageUrl={previewImage} handleAddImage={(e: any) => setFile(e.target.files?.[0])} />
          </ToggleCollapse>

          <ToggleCollapse header={t('SCREEN.SELLER_REGISTRATION.REVIEWS_SUMMARY_LABEL')} open={false}>
            <TrustMeter ratings={dbUserSettings?.trust_meter_rating || 0} />
          ...
          {isOnlineShoppingEnabled && (
            <>
              ...
              <ToggleCollapse header={t('SCREEN.SELLER_REGISTRATION.SELLER_ONLINE_SHOPPING_ORDER_FULFILLMENT_LABEL')} open={false}>
                {/* استخدام المكون المستورد بشكل صحيح لضمان عدم كسر الـ Build */}
